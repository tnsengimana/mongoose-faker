...
let addToSession = (obj) => {
  if (sessions.length > 0) {
    sessions[sessions.length - 1].push(obj);
  }
}
...
let destroySession = async () => {
  if (sessions.length == 0) {
    return;
  }

  const objectsToRemove = sessions[sessions.length - 1];
  for (let i = 0; i < objectsToRemove.length; i++) {
    const obj = objectsToRemove[i];
    const res = await obj.remove();
  }
